...
const infoString = "goboy snapshot"
...
func (cs *cpuState) loadSnapshot(snapBytes []byte) (*cpuState, error) {
	...
	if reader, err = gzip.NewReader(bytes.NewReader(snapBytes)); err != nil {
		return nil, err
	} else if unpackedBytes, err = io.ReadAll(reader); err != nil {
		return nil, err
		...
		return nil, fmt.Errorf("this version of goboy is too old to open this snapshot")
	}
